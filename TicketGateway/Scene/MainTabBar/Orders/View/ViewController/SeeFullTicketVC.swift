...
class SeeFullTicketVC: UIViewController {
    ...
    func setFont() {
        ...
        for dottedLine in dottedLines {
            dottedLine?.createDottedLine(width: 2, color: UIColor.setColor(colorType: .BorderLineColour).cgColor, dashPattern: [6,3])
        }
...
//      self.btnSeeLessView.addTarget(self, action: #selector(buttonPressed(_:)), for: .touchUpInside)
        ...
        [self.btnGetARefund,self.btnSeeLessView,self.btnSaveTicketAsImage,self.btnAddAppToWallet].forEach {
